...
const aside_bar = document.querySelector("aside .pointer-container");
...
function elementIsVisible(el, partiallyVisible = true) {
  ...
  return partiallyVisible
    ? ((top > 0 && top < innerHeight) || (bottom - 200 > 0 && bottom < innerHeight)) && ((left > 0 && left < innerWidth) || (right > 0 && right < innerWidth))
...
document.addEventListener("scroll", () => {
  ...
  for (let i = 0; i < content_titles.length; i++) {
    if ((elementIsVisible(content_titles[i]), elementIsVisible(contents[i]))) {
